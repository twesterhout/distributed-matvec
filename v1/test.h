...
typedef enum { /* Initially fill up the search subspace with: */
               primme_init_default,
               primme_init_krylov, /* a) Krylov with the last vector provided by
                                      the user or random */
...
typedef struct primme_stats {
  ...
  double estimateLargestSVal; /* absolute value of the farthest to zero Ritz
                                 value seen */
...
typedef struct primme_params {
  ...
  PRIMME_INT n;
  void (*matrixMatvec)(void *x, PRIMME_INT *ldx, void *y, PRIMME_INT *ldy,
  ...
  void (*applyPreconditioner)(void *x, PRIMME_INT *ldx, void *y,
  ...
  void (*massMatrixMatvec)(void *x, PRIMME_INT *ldx, void *y, PRIMME_INT *ldy,
  ...
  PRIMME_INT nLocal;
  void *commInfo;
  void (*globalSumReal)(void *sendBuf, void *recvBuf, int *count,
                        struct primme_params *primme, int *ierr);
  primme_op_datatype
      globalSumReal_type; /* expected type of sendBuf and recvBuf */
  void (*broadcastReal)(void *buffer, int *count, struct primme_params *primme,
                        int *ierr);
  ...
  primme_orth orth;
  primme_op_datatype
      internalPrecision; /* force primme to work in that precision */
  ...
  void (*monitorFun)(void *basisEvals, int *basisSize, int *basisFlags,
                     ...
                     int *numConverged, void *lockedEvals, int *numLocked,
                     int *lockedFlags, void *lockedNorms, int *inner_its,
                     void *LSRes, const char *msg, double *time,
                     primme_event *event, struct primme_params *primme,
                     int *err);
